...
def get_callbacks(model, group, position, size):
    """
    :return: A list of `keras.callbacks.Callback` instances to apply during training.

    """
    path = 'models/{group}/{position}/{model}/{size}/'.format(
        group=group,
        position=position,
        model=model,
        size=size)
...
def get_train_datagen(model, size):
...
def train_from_scratch(group, position, size, n_epochs):
    ...
    full_model.fit_generator(
        ...
        epochs=n_epochs,
        verbose=1,
        callbacks=get_callbacks('scratch', group, position, size),
        validation_data=test_generator,
        validation_steps=np.ceil(n_test_samples / batch_size),
        class_weight=class_weight,
        max_q_size=10,
...
def main():
    ...
    parser.add_argument('--model', default='scratch', help='The network eg. resnet50')
    parser.add_argument('--size', default=1024, help='Image dimension')
