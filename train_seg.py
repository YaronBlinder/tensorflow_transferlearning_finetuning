...
def get_callbacks(seg, model, top, group, position, train_type, n_dense=512, dropout=False, **kwargs):
    """
...
def pneumo(model, top, group, position, size, n_epochs, n_dense, dropout, pooling, G, seg):
    ...
    if G > 1:
        ...
        full_model.fit_generator(
            ...
            epochs=n_epochs,
            verbose=1,
            callbacks=get_callbacks(seg, model, top, group, position, train_type, n_dense, dropout, G=G),
            validation_data=test_generator,
            validation_steps=int(np.ceil(n_test_samples / (batch_size * G))),
            class_weight=class_weight,
            max_queue_size=10,
